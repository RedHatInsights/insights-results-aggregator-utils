...
func drawMarchingAnts(img *image.Paletted, step int) {
	...
	for y := 388; y < 510; y += 20 {
		drawAnt(img, 904, y+step, 1)
	}

	// first horizontal line
	for x := 904; x > 798; x -= 20 {
		drawAnt(img, x-step, 530, 1)
	}
	...
	drawAnt(img, 472-12, 212, 3)
	drawAnt(img, 561, 212, 3)
	drawAnt(img, 613-12, 212, 3)
	drawAnt(img, 717, 212, 3)
	drawAnt(img, 727, 212, 3)
	drawAnt(img, 820-35, 212, 3)
	drawAnt(img, 820-25, 212, 3)
}
