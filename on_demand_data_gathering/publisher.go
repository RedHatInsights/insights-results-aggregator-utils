...
const (
	...
	recordDuration       = "10000s" // should be "86400s" in production
	recordingDelay       = 200 * time.Millisecond
	minRuleHits          = 1
	maxRuleHits          = 10
	clusterNamesFilename = "cluster_names.txt" // file that contains all cluster names that should be used
	storeRuleHits        = false               // trick to store just unique report keys, not values
	includeOrgIDInKey    = true                // it is possible to turn on or turn off organization ID as a prefix
	minimumOrgID         = 1                   // minimum org ID value (testing org ID is generated by RNG)
	maximumOrgID         = 1000                // maximum org ID value (testing org ID is generated by RNG)
)
...
func generateOrgPrefix(min, max int) string {
	return strconv.FormatUint(uint64(rand.Intn(max-min)+min), 10)
}

func generateReportKey(includeOrgIDInKey bool, minimumOrgID, maximumOrgID int, clusterNames []string) string {
	orgPrefix := ""
	if includeOrgIDInKey {
		orgPrefix = generateOrgPrefix(minimumOrgID, maximumOrgID)
	}
	return orgPrefix + "." + generateClusterName(clusterNames) + "." + generateTrackerID()
}
...
func main() {
	...
	for {
		// generate report
		key := generateReportKey(includeOrgIDInKey, minimumOrgID, maximumOrgID, clusterNames)
